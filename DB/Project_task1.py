...
decision = input('What would you like to do? (create_db, create_tbl, insert_data, get_data, delete_tbl, alter_tbl, drop_tbl, drop_db): ')
...
if decision == 'create_db':
	...
	col_num = int(input('Please provide number of columns: '))
	db_cursor.execute(f"CREATE TABLE IF NOT EXISTS db_project.{table_name} (ID INT AUTO_INCREMENT PRIMARY KEY);")
	for i in range(col_num):
		column_name = input('Please provide name of the column: ')
		column_type = input('Please provide column  data type: ')
		db_cursor.execute(f"ALTER TABLE db_project.{table_name} ADD {column_name} {column_type};")

elif decision == 'insert_data':
	db_cursor.execute("INSERT INTO db_project.test(`ID`, `aa`, `bb`, `cc`) VALUES (NULL,'aaaa','bbbb','cccc');")
	...
	if is_sorted == 'Y':
		db_cursor.execute(f'SHOW COLUMNS FROM db_project.{table_name};')
		column_name = input('Please select name of the column you want to order by: ')
		db_cursor.execute(f'SELECT * FROM db_project.{table_name} ORDER BY {column_name} ASC;')
	elif is_sorted == 'N':
		db_cursor.execute(f'SELECT * FROM db_project.{table_name};')

elif decision == 'delete_tbl':
	table_name = input('Please provide name of the table you would like to delete: ')
	db_cursor.execute(f'DELETE * FROM TABLE db_project.{table_name};')
	...
	if alter_operation == 'ADD':
		...
		db_cursor.execute(f"ALTER TABLE db_project.{table_name} DROP COLUMN {column_name};")
	elif alter_operation == 'MODIFY':
		column_name = input('Please provide name of the column to modify: ')
		column_type = input('Please provide column data type: ')
		db_cursor.execute(f"ALTER TABLE db_project.{table_name} MODIFY COLUMN {column_name} {column_type};")

elif decision == 'drop_tbl':
	table_name = input('Please provide name of the table you would like to drop: ')
	db_cursor.execute(f"DROP TABLE db_project.{table_name};")
